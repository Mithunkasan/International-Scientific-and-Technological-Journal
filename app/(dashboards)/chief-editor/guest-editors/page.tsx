...
export default async function GuestEditorsPage() {
  ...
  // Get active guest editors (Role.GUEST_EDITOR)
  const editors = await prisma.user.findMany({
    where: {
      role: Role.GUEST_EDITOR,
      isActive: true,
    },
    orderBy: { name: 'asc' },
  });
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1 font-medium">
          List of active registered guest editors for specific themed journal issues.
        </p>
      </div>
      ...
      <Card className="border-border shadow-xs overflow-hidden">
        <CardHeader className="bg-card px-6 py-4 border-b border-border">
          <CardTitle className="text-base font-bold text-foreground tracking-tight uppercase">
            Active Guest Editors ({editors.length})
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {editors.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground text-sm font-medium flex flex-col items-center justify-center gap-2">
              ...
              <span>No guest editors registered yet.</span>
            </div>
          ) : (
